...
// Arabic core block + supplement + Arabic Presentation Forms A/B
const ARABIC_RE = /[\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFF]/;

function isValidArabicTitle(title: string | undefined | null): boolean {
    return !!title && title.trim().length > 0 && ARABIC_RE.test(title);
}

/** Removes, in place, any article with no title or a non-Arabic title. Returns count removed. */
function removeInvalidTitleArticles(data: ArticlesConfig): number {
    let removed = 0;
    for (const cat of data.articles) {
        const before = cat.content.length;
        cat.content = cat.content.filter((a) => {
            const ok = isValidArabicTitle(a.title);
            if (!ok) {
                const reason = !a.title?.trim() ? 'no title' : 'non-Arabic title';
                console.log(`🗑️  [${cat.category}] Removing (${reason}): "${a.title || ''}" [${a.id_str}]`);
            }
            return ok;
        });
        removed += before - cat.content.length;
    }
    return removed;
}
...
async function processItem(
    ...
    if (!isValidArabicTitle(item.title)) {
        const reason = !item.title?.trim() ? 'no title' : 'non-Arabic title';
        console.log(`⏭  ${label} Skip (${reason}): "${(item.title || '').slice(0, 55)}"`);
        return false;
    }
