 ...
 *   3. You can also hardcode `feedUrl` per blog in personal-blogs.ts if a site's
 ...
 *
 * Every run re-checks every blog already registered in src/data/personal-blogs.ts,
 ...
 * add them to personal-blogs.ts to have them re-checked on future runs).
 *
 * Usage:
 *   bun run scripts/import-personal-blogs.ts [url] [url2] ...
 *
 * Example:
 *   bun run scripts/import-personal-blogs.ts                                     # re-check all known blogs
 *   bun run scripts/import-personal-blogs.ts https://alfarhan.ws                 # + a brand-new blog
 *   bun run scripts/import-personal-blogs.ts https://alfarhan.ws https://fatthatmablog.wordpress.com
 *
...
const knownJobs: BlogJob[] = Object.values(personalBlogs).map(b => ({
    slug: b.slug,
    name: b.name,
    url: b.url,
    feedUrl: b.feedUrl,
...
if (jobs.length === 0) {
    console.error('Usage: bun run scripts/import-personal-blogs.ts [url] [url2] ...');
    console.error('Example: bun run scripts/import-personal-blogs.ts https://alfarhan.ws');
    console.error('(with no args, re-checks every blog already in src/data/personal-blogs.ts)');
    process.exit(1);
}

console.log(
    `📚 Checking ${jobs.length} blog(s) total ` +
    `(${knownJobs.length} already known from personal-blogs.ts` +
    (cliJobs.length > 0 ? `, ${cliJobs.length} new from CLI)` : ')') +
    '\n'
);

if (cliJobs.length > 0) {
    console.log('💡 New blog(s) — add these to src/data/personal-blogs.ts to have them re-checked on future runs:');
